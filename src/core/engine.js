...
export class TickEngine{
  ...
  register(sys){
    ...
    for(const C of sys.writes||[]){
      const k=sys.phase+':'+C.name
      if(this.byPhase.has(k)) throw new Error(`CONFLICT ${sys.constructor.name} vs ${this.byPhase.get(k)} on ${C.name} in ${sys.phase}`)
      this.byPhase.set(k, sys.constructor.name)
    }
