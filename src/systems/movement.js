...
export class MovementSystem{
  ...
  update(world,events,dt){
    const provPos=new Map()
    for(const[,[p]] of world.query(Position,Province)) provPos.set(p.provinceId,p)
    const orders=events.drain('order_move')
    for(const o of orders){
      const pos=world.getComponent(o.entityId,Position); if(!pos) continue
      const t=provPos.get(o.targetProvinceId); if(!t) continue
    ...
    for(const[eid,[pos,army]] of world.query(Position,Army)){
      ...
      if(pos._tx!==undefined){
        ...
        if(d<3){ delete pos._tx; delete pos._ty; delete pos._speed } else { const s=(pos._speed||1.2)*2.4*dt; pos.x+=dx/d*s; pos.y+=dy/d*s; moved++ }
