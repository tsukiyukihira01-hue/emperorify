...
export class MovementSystem {
  ...
  update(world, events, dt){
    ...
    for(const [, [pPos, prov]] of world.query(Position, Province)){
    ...
    for(const o of orders){
      ...
      pos._tx = target.x + 20
      pos._ty = target.y + 50
      pos._speed = o.speed || 1.2
      pos.provinceId = o.targetProvinceId
    }
    ...
    for(const [eid, [pos, army]] of world.query(Position, Army)){
      ...
      if(pos._tx !== undefined){
        ...
        if(dist < 3){
          ...
          const s = (pos._speed||1.0) * 2.4 * dt
          pos.x += (dx/dist)*s
          pos.y += (dy/dist)*s
          moved++
        }
      } else {
        pos.x += Math.sin(eid + Date.now()*0.00008)*0.05*dt
        pos.y += Math.cos(eid + Date.now()*0.00008)*0.05*dt
      }
    }
