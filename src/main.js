...
import { Position, Army, Province, Economy, Relation, TradeRoute } from './components/index.js'
...
function updateStats(){
  ...
  const gold=[...world.query(Economy)].reduce((s,[, [e]])=>s+e.gold,0)|0
  if(statsEl) statsEl.textContent=`A:${a} P:${p} Rel:${r} Trade:${tr} Gold:${gold} T:${engine.tickCount} | ${engine.policy.dump()}`
}
...
events.on('war_declared',d=>log(`WAR ${d.from}->${d.to} trade blocked`))
events.on('peace_made',d=>log(`Peace ${d.from}<->${d.to} trade resumes`))
events.on('trade_created',d=>log(`Trade route +${d.created}`))
events.on('trade_blocked',d=>log(`Trade blocked ${d.blocked}`))
events.on('trade_income',d=>log(`Trade income ${d.value} from ${d.routes} routes`))
...
document.getElementById('save').onclick=async()=>{ try{ await repo.bulkSave(world); log('Saved v2 with trade') }catch(e){ log('Save err '+e.message) } }
...
log('READY diplomacy+trade safe')
